...
local Window = Rayfield:CreateWindow({
   Name = "BulletWareV5 | Premium",
   LoadingTitle = "Thanks for buying premium",
   LoadingSubtitle = "Made by ShootasWrld",
   ConfigurationSaving = {
      Enabled = true,
...
_G.legitscript = false
_G.glovehits = true
_G.cloudSpeed = false
_G.larrySpeed = 30
_G.spamreplicaballer = false
_G.autoplayer = false
...
local settings_tbl = {
    ...
    Chams_Glow_Color = Color3.fromRGB(0,0,255)
}
...
function bobMorph()
    ...
    char.Head.Nametag.TextLabel.Text = "B̸̺̗̣̝̤̞͍͍̖̘̤̑̍͑͐̎̓̈́Ö̷̡͙̲̭̠̹̪̦̋̅͑̈́͝Ḇ̵̨̡̧̰͚͍̾͌"
    ...
    function robMorph()
        ...
        char.Head.Mesh.Scale = Vector3.new(1.25, 1.25, 1.25)
        char.Head.face.Texture = "rbxassetid://4114747358"
        char.Head.Nametag.TextLabel.Text = "R̷̡̛̛̦͉̘̻͖͇̳͖̦̔̈́͒̐̽̽̾ǫ̷̰̼͙̻̘͖̖̗̌̈́̅̽̌̅̋͠b̴̡͇̣̗̥̠̘͎̫͔̆͛̓̅͛"
        
        end

    function cloudSpeed()
    local player = game.Players.LocalPlayer
    while _G.cloudSpeed == true do
    local cloud = game.Workspace:FindFirstChild(player.Name .. "_Cloud")
    cloud.BodyVelocity.Velocity = cloud.BodyVelocity.Velocity * 1.1
    wait(0.05)
    end
    end
    ...
    for _, model in pairs(workspace:GetDescendants()) do
        if model:IsA("Model") and model:FindFirstChild("Skelly") then
            model.VehicleSeat.LinearVelocity.LineVelocity = _G.larrySpeed
        end
    end
    wait(0.05)
    end
...
task.wait()
    ...
    local Toggle = MainTab:CreateToggle({
    Name = "Replica & Baller Slapfarm",
    CurrentValue = false,
    Flag = "RBSF", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Value)
        ...
        elseif ReplicaBallerFarm == true then
            Rayfield:Notify({
                Title = "Slapfarm Error",
                Content = "Baller Isnt Equipped!",
                Duration = 3.5,
                Image = nil,
                Actions = { -- Notification Buttons
                   Ignore = {
...
local Toggle = MainTab:CreateToggle({
    Name = "Replica Slapfarm (only default arena)",
    CurrentValue = false,
    Flag = "SlapFarm", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Value)
        ReplicaFarm = Value
...
elseif ReplicaFarm == true then
    Rayfield:Notify({
        Title = "Error",
        Content = "You dont have replica equipped or you arent in default arena",
        Duration = 6.5,
        Image = nil,
        Actions = { -- Notification Buttons
           Ignore = {
    ...
    Name = "Auto Player",
    CurrentValue = false,
    Flag = "AutoPlayer", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Value)
    _G.autoplayer = Value
    _G.autoplayer()
    end,
...
local Button = MainTab:CreateButton({
    Name = "GodmodeV2 (faster) (No Glove)",
    Callback = function()
        if game.Players.LocalPlayer.Character:FindFirstChild("entered") == nil then
            firetouchinterest(game.Players.LocalPlayer.Character:WaitForChild("Head"), workspace.Lobby.Teleport1.TouchInterest.Parent, 0)
            firetouchinterest(game.Players.LocalPlayer.Character:WaitForChild("Head"), workspace.Lobby.Teleport1.TouchInterest.Parent, 1)
 ...
 local Input = MainTab:CreateInput({
    ...
    Callback = function(Text)
        if game.Players.LocalPlayer.Character:FindFirstChild("Swapper") or game.Players.LocalPlayer.Backpack:FindFirstChild("Swapper") then
            ...
            if workspace[Text]:FindFirstChild("HumanoidRootPart") then
            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(workspace[Text].HumanoidRootPart.Position.X,-49999,workspace[Text].HumanoidRootPart.Position.Z)
            end
            task.wait(0.01)
            if Timer < 1 then
            Timer = Timer + 0.01
            end
            until game.Players[Text].Character and workspace[Text]:FindFirstChild("HumanoidRootPart") and workspace[Text]:FindFirstChild("entered") and workspace[Text].Ragdolled.Text == false and Timer >= 1
            ...
            CoreGui:SetCore("SendNotification", {
                ...
                Text = "You dont have swapper equipped",
                Duration = 3,
                })
        end
    end,
...
local Button = MainTab:CreateButton({
    ...
    Suffix = "WalkSpeed",
    CurrentValue = 10,
    Flag = "FASTER", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Value)
    local bypass;
 ...
 local Slider = MainTab:CreateSlider({
    ...
    Suffix = "JumpPower",
    CurrentValue = 10,
    Flag = "JUMPY", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Value)
    ...
    local Toggle = MainTab:CreateToggle({
        Name = "Change Nametag (clientsided)",
        CurrentValue = false,
        Flag = "NAMETAG", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
        Callback = function(Value)
    ...
    repeat task.wait() until game.Players.LocalPlayer.Character:FindFirstChild("Nametag",true)
     ...
     local Input = MainTab:CreateInput({
        Name = "Change Nametag (client sided)",
        PlaceholderText = "Nametag",
        RemoveTextAfterFocusLost = false,
        Callback = function(Text)
        workspace.NametagChanged.Text = Text
        end,
     })
     ...
     local Toggle = MainTab:CreateToggle({
        Name = "Remove Nametag",
        CurrentValue = false,
        Flag = "NONAME", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
        Callback = function(Value)
            if Value then
                local vis = true
...
local Button = MiscTab:CreateButton({
    Name = "Candy Corn Farm (Copies script put in autoexec)",
    Callback = function()
...
local Toggle = MiscTab:CreateToggle({
 ...
 local Button = MiscTab:CreateButton({
    Name = "Quake Nuke (quake Glove needed)",
    Callback = function()
        while wait() do
            local args = {
                [1] = {
 ...
 local Input = MiscTab:CreateInput({
    Name = "skukuchi Abuse (player)",
    PlaceholderText = "Username",
    RemoveTextAfterFocusLost = false,
    Callback = function(Text)
    _G.skukuchiPlayer = Text
    end,
 })

 local Toggle = MiscTab:CreateToggle({
    Name = "Defense Nuke (defense glove needed)",
    CurrentValue = false,
    Flag = "defense", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Value)
        getgenv().DefenseExploit = Value
        game:GetService("RunService").heartbeat:Connect(function()
                while true do
...
game:GetService("RunService").heartbeat:Connect(function()
 ...
 local Dropdown = MiscTab:CreateDropdown({
    ...
    Flag = "BOB OR ROB", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Option)
    if Option == "Bob" then
    bobMorph()
    elseif Option == "Rob" then
    RobMorph()
        end
    end,
 })
...
local Toggle = MiscTab:CreateToggle({
    ...
    CurrentValue = false,
    Flag = "CANDY", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Value)
        CandyCornFarm = Value
...
start()
 ...
 local Input = MiscTab:CreateInput({
    ...
    Callback = function(Text)
        ...
        if getgenv().feed == true and getgenv().target ~= nil and game.Players:FindFirstChild(getgenv().target) then
        
        local tofeed = game.Players:FindFirstChild(getgenv().target)
        local feedchar = tofeed.Character or tofeed.CharacterAdded:Wait()
        local targethrp = feedchar:WaitForChild("HumanoidRootPart", 50)
        local tpgate = workspace.Lobby:WaitForChild("Teleport1")
        firetouchinterest(tpgate, hrp, 0)
        firetouchinterest(tpgate, hrp, 1)
        hrp.CFrame = targethrp.CFrame
        repeat wait(1)
            hrp.CFrame = targethrp.CFrame 
        until char:FindFirstChild("Ragdolled") and char:FindFirstChild("Ragdolled").Value == true
 ...
 local Slider = MiscTab:CreateSlider({
    Name = "Larry Walkspeed",
    Range = {0, 250},
    Increment = 1,
    Suffix = "Speed",
    CurrentValue = 30,
    Flag = "LARRY", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Value)
    _G.larrySpeed = Value
	larrySpeed()
    end,
...
local Toggle = MiscTab:CreateToggle({
    Name = "Give Godmode",
    CurrentValue = false,
    Flag = "GG", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Value)
        GiveGodmode = Value
        ...
        elseif GiveGodmode == true then
            Rayfield:Notify({
                Title = "Give Godmode error",
                Content = "Player Not found or Guarding angel not equipped",
                Duration = 4,
                Image = nil,
                Actions = { -- Notification Buttons
                   Ignore = {
...
local Input = MiscTab:CreateInput({
    Name = "Equip Glove (Only Lobby or Godmode)",
    PlaceholderText = "Glove Name",
    RemoveTextAfterFocusLost = true,
    Callback = function(Text)
    if game.Players.LocalPlayer.Character:FindFirstChild("entered") == nil then
    fireclickdetector(workspace.Lobby[Text].ClickDetector)
    end
    end,
 })
...
local Toggle = MiscTab:CreateToggle({
    Name = "Infinite Phase",
    CurrentValue = false,
    Flag = "Phase", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Value)
    _G.infPhase = Value
    infinitePhase()
    end,
 ...
 local Button = MiscTab:CreateButton({
    Name = "Infinite Ingredients (Alchemist needed)",
    Callback = function()
    ingredients()
    end,
 })
...
local Button = BadgesTab:CreateButton({
   Name = "Get Counter (only use in elude maze)",
   Callback = function()
...
local Button = BadgesTab:CreateButton({
 ...
 local Button = BadgesTab:CreateButton({
    ...
    Callback = function()
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = workspace.FinishDoor_Retro.Part.CFrame    end,
 })
...
local Button = PremiumTab:CreateButton({
    ...
    Callback = function()
        ...
        CoreGui:SetCore("SendNotification", {
            Title = "Tutorial to use",
            Text = "Press Brazil Kick button and teleport to player you want to kick.",
            Duration = 5,
        })
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-479, 309, -1)
        wait(0.001)
...
local Input = PremiumTab:CreateInput({
    Name = "bring Player (type there user)",
    PlaceholderText = "Username",
    RemoveTextAfterFocusLost = false,
    Callback = function(Text)
    _G.recallPlayer = Text
    end,
 })

 local Button = PremiumTab:CreateButton({
    Name = "Bring Player (recall needed)",
    Callback = function()
        local CoreGui = game:GetService("StarterGui")
        CoreGui:SetCore("SendNotification", {
        Title = "Tutorial",
        Text = "Type the players users that you want to bring and then click this button in arena",
        Duration = 3,
       })
    recallTP()
    end,
